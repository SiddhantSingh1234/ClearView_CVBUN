...
df = pd.read_csv('Combined.csv')  # Change 'input.csv' to your actual filename
...
dataset = dataset.map(lambda x: {"labels": torch.tensor(x["label"], dtype=torch.long)}, batched=False)
dataset = dataset.map(tokenize_function, batched=True)
dataset = dataset.train_test_split(test_size=0.2, shuffle=True)
test_valid = dataset["test"].train_test_split(test_size=0.5, shuffle=True)
...
model.save_pretrained("fine_tuned_fake_news_roberta")
tokenizer.save_pretrained("fine_tuned_fake_news_roberta")
